...
class ProjectEndpoint(implicit inj: Injector) extends HttpService
  ...
  val indirectGetProjects = path("extendedProjects") & get & parameters('cultureId.as[BSONObjectID] ?, 'speciesId.as[BSONObjectID] ?, 'containerId ?, 'locationId.as[BSONObjectID] ?, 'contaminated.as[Boolean] ?)
  ...
  val route = {
    ...
    authenticate(authenticator.basicAuthenticator) { user =>
      ...
      indirectGetProjects { (cultureId, speciesId, containerId, locationId, contaminated) =>
        complete {
          service.search(cultureId, speciesId, containerId, locationId, contaminated, user._id)
        }
      } ~
